...
LightEditorDialog::LightEditorDialog(QWidget *parent) : QDialog(parent) {
  ...
  posX_ = new QDoubleSpinBox;
  posX_->setRange(-100, 100);
  posX_->setDecimals(2);
  posY_ = new QDoubleSpinBox;
  posY_->setRange(-100, 100);
  posY_->setDecimals(2);
  posZ_ = new QDoubleSpinBox;
  posZ_->setRange(-100, 100);
  posZ_->setDecimals(2);
  ...
  ambientBtn_ = new QPushButton;
  ambientBtn_->setFixedSize(40, 20);
  connect(ambientBtn_, &QPushButton::clicked, this,
          &LightEditorDialog::onAmbientClicked);
  colorForm->addRow("Ambient:", ambientBtn_);

  diffuseBtn_ = new QPushButton;
  diffuseBtn_->setFixedSize(40, 20);
  connect(diffuseBtn_, &QPushButton::clicked, this,
          &LightEditorDialog::onDiffuseClicked);
  colorForm->addRow("Diffuse:", diffuseBtn_);

  specularBtn_ = new QPushButton;
  specularBtn_->setFixedSize(40, 20);
  connect(specularBtn_, &QPushButton::clicked, this,
          &LightEditorDialog::onSpecularClicked);
...
void LightEditorDialog::updateButtonColor(QPushButton *btn,
                                          const QColor &color) {
...
void LightEditorDialog::onAmbientClicked() {
  ...
  if (c.isValid()) {
    ambientColor_ = c;
    updateButtonColor(ambientBtn_, c);
  }
}
void LightEditorDialog::onDiffuseClicked() {
  ...
  if (c.isValid()) {
    diffuseColor_ = c;
    updateButtonColor(diffuseBtn_, c);
  }
}
void LightEditorDialog::onSpecularClicked() {
  ...
  if (c.isValid()) {
    specularColor_ = c;
    updateButtonColor(specularBtn_, c);
  }
}
...
void LightEditorDialog::setLight(const LightSource &light) {
  ...
  ambientColor_ =
      QColor::fromRgbF(light.ambient.r, light.ambient.g, light.ambient.b);
  diffuseColor_ =
      QColor::fromRgbF(light.diffuse.r, light.diffuse.g, light.diffuse.b);
  specularColor_ =
      QColor::fromRgbF(light.specular.r, light.specular.g, light.specular.b);
  updateButtonColor(ambientBtn_, ambientColor_);
  updateButtonColor(diffuseBtn_, diffuseColor_);
  updateButtonColor(specularBtn_, specularColor_);
}
...
LightSource LightEditorDialog::getLight() const {
  ...
  l.ambient = glm::vec3(ambientColor_.redF(), ambientColor_.greenF(),
                        ambientColor_.blueF());
  l.diffuse = glm::vec3(diffuseColor_.redF(), diffuseColor_.greenF(),
                        diffuseColor_.blueF());
  l.specular = glm::vec3(specularColor_.redF(), specularColor_.greenF(),
