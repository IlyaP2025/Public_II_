...
std::unique_ptr<KdTreeMeshIndex::Node> KdTreeMeshIndex::BuildRecursive(
    std::vector<std::pair<BoundingBox, size_t>> items, int depth) {
  ...
  if (items.size() <= kLeafMaxItems || depth >= kMaxDepth) {
    ...
    for (const auto& item : items) node->indices.push_back(item.second);
    return node;
  }
...
void KdTreeMeshIndex::QueryRecursive(const Node* node, const Point& origin,
                                     const Point& direction,
                                     std::vector<size_t>& result) const {
  if (!node) return;

  if (!node->bbox.IntersectsRay(origin, direction)) return;
