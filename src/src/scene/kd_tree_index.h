...
private:
    ...
    std::unique_ptr<Node> BuildRecursive(std::vector<std::pair<BoundingBox, size_t>> items,
                                         int depth);
    void QueryRecursive(const Node* node, const Point& origin, const Point& direction,
